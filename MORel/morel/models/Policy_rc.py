...
class ActorCriticPolicy(nn.Module):
    def __init__(self, input_dim,
        ...
        self.var = torch.nn.Parameter(torch.tensor([0.0,0.0]).to(self.device), requires_grad = True)
...
class PPO2():
    def __init__(self, input_dim, output_dim, network = ActorCriticPolicy, use_wandb=True):
    ...
    def generate_experience(self, env, n_steps, gamma, lam, eval_render=True):
            ...
            for i in range(n_steps):
                if(done):
                    ...
                    if(not isinstance(env, FakeEnv)):
                        ...
                        obs_copy_2 = copy.deepcopy(obs)
                        obs = torch.from_numpy(obs_copy[0]['observation']).float().to(self.device)
                        #obs = torch.from_numpy(obs_copy).float().to(self.device)
                        #print('obs from real env ', obs)
                        
                    obs = torch.unsqueeze(obs, 0)
                    #print('obs unsqueezed', obs)

                    info["episode_rewards"].append(total_reward)
                    total_reward = 0
                
                if(not isinstance(env, FakeEnv)):
                    eval_reward_list.append(obs_copy_2[0]['desired_goal'])

                #print('generative interation', i)
                # Choose action
                action, neg_log_prob, _, value = self.forward(observation = obs)

                #print('action pre_squeeze', action)

                # Retrieve values
                action = torch.squeeze(action)
                #print('action post_squeeze', action)
                ...
                if(isinstance(env, FakeEnv)):
                    ...
                    if done:
                        halted_locations.append(obs)
                else:
                    #print('action', action.cpu().numpy())
                    obs, rewards, done, truncated, env_info = env.step(action.cpu().numpy())
                    if eval_render:
                        env.render()
                   
                    if done:
                        print('EVAL had a DONE!!')
                        print("RESETTING ENV within the run!!")
                        
                    obs_copy = copy.deepcopy(obs)
                    obs = torch.from_numpy(obs_copy['observation']).float().to(self.device)
        ...
        return mb_rewards, mb_obs, mb_returns, mb_done, mb_actions, mb_values, mb_neg_log_prob, info, halted_locations, eval_reward_list
    ...
    def train(self, env, optimizer = torch.optim.Adam,
        ...
        halted_locations_list = []

        # main train loop
        for update in tqdm(range(n_updates)):
            # Collect new experiences using the current policy
            rewards, obs, returns, dones, actions, values, neg_log_probs, info, halted_locations, _ = self.generate_experience(env, n_steps, gamma, lam)
            halted_locations_list.append(halted_locations)
            ...
            for i in range(num_train_epochs):
                ...
                if self.use_wandb:
                        wandb.log({
                            ...
                            'Metrics/max_reward': sum(info["episode_rewards"])/len(info["episode_rewards"]),  
                            'Metrics/halt_per_episode': info["HALT"]/len(info["episode_rewards"]) 
                        })
        
        with open('halted_locations_list_train', 'wb') as file:
            print("Saving halted locations!!")
            pickle.dump(halted_locations_list, file)
    ...
    def eval(self, obs):
        ...
        with torch.set_grad_enabled(False):
            ...
            if self.use_wandb:
                self.wandb_run.log({'Eval/action_mean': action.mean().item()})
