...
class DynamicsNet(nn.Module):
    def __init__(self, input_dim, output_dim, n_neurons = 512, activation = nn.ReLU, seed=None):
...
class DynamicsEnsemble():
    def __init__(self, input_dim, output_dim, n_models=4, n_neurons=512, threshold=1.0, n_layers=2, activation=nn.ReLU, cuda=False, use_wandb=True ):
        ...
        for i in range(n_models):
            model_seed = SEED + i
            model = DynamicsNet(input_dim, output_dim, n_neurons=n_neurons, activation=activation, seed=model_seed).to(device)
    ...
    def train(self, dataloader, epochs = 10, optimizer = torch.optim.Adam, loss = nn.MSELoss):
        ...
        for epoch in range(epochs):
            for i, batch in enumerate(tqdm(dataloader)):
                ...
                if self.use_wandb:  # Log to wandb if the run is provided
                    wandb.log({'epoch': epoch, 'batch_idx': i, 
                               **{f'loss_model_{i}': loss_val for i, loss_val in enumerate(loss_vals)}})
